		...
		if (autoparts_exists_woocommerce()) {
			...
			autoparts_storage_merge_array('options', '', array(
				...
				'posts_per_page_shop' => array(
					"title" => esc_html__('Products per page', 'autoparts'),
					"desc" => wp_kses_data( __('How many products should be displayed on the shop page. If empty - use global value from the menu Settings - Reading', 'autoparts') ),
					"std" => '',
					"type" => "text"
					),
				'blog_columns_shop' => array(
					"title" => esc_html__('Shop loop columns', 'autoparts'),
					"desc" => wp_kses_data( __('How many columns should be used in the shop loop (from 2 to 4)?', 'autoparts') ),
					"std" => 2,
...
if (!function_exists('autoparts_woocommerce_theme_setup9')) {
	...
	function autoparts_woocommerce_theme_setup9() {
		...
		if (autoparts_exists_woocommerce()) {
			...
			add_filter(    'loop_shop_columns',							'autoparts_woocommerce_loop_shop_columns' );
			add_filter(    'post_class',								'autoparts_woocommerce_loop_shop_columns_class' );
			add_filter(    'product_cat_class',							'autoparts_woocommerce_loop_shop_columns_class', 10, 3 );
...
/*			add_action(    'woocommerce_before_shop_loop_item_title',	'autoparts_woocommerce_title_tags', 30 ); */
			...
			// Decorate price
/*			add_filter(    'woocommerce_get_price_html',				'autoparts_woocommerce_get_price_html' );*/
...
if ( !function_exists( 'autoparts_woocommerce_detect_blog_mode' ) ) {
	//Handler of the add_filter( 'autoparts_filter_detect_blog_mode', 'autoparts_woocommerce_detect_blog_mode' );
...
if ( !function_exists( 'autoparts_woocommerce_post_type_taxonomy' ) ) {
	//Handler of the add_filter( 'autoparts_filter_post_type_taxonomy',	'autoparts_woocommerce_post_type_taxonomy', 10, 2 );
...
if ( !function_exists( 'autoparts_woocommerce_allow_override_header_image' ) ) {
	//Handler of the add_filter( 'autoparts_filter_allow_override_header_image', 'autoparts_woocommerce_allow_override_header_image' );
...
if ( !function_exists( 'autoparts_woocommerce_get_post_categories' ) ) {
	//Handler of the add_filter( 'autoparts_filter_get_post_categories', 		'autoparts_woocommerce_get_post_categories');
...
if ( !function_exists( 'autoparts_woocommerce_list_post_types' ) ) {
	//Handler of the add_filter( 'autoparts_filter_list_posts_types', 'autoparts_woocommerce_list_post_types');
...
if ( !function_exists( 'autoparts_woocommerce_get_post_info' ) ) {
	//Handler of the add_filter( 'autoparts_filter_get_post_info', 'autoparts_woocommerce_get_post_info');
...
if ( !function_exists( 'autoparts_woocommerce_action_before_post_meta' ) ) {
	//Handler of the add_action( 'autoparts_action_before_post_meta', 'autoparts_woocommerce_action_before_post_meta');
...
if ( !function_exists( 'autoparts_woocommerce_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'autoparts_woocommerce_frontend_scripts', 1100 );
	function autoparts_woocommerce_frontend_scripts() {
		//if (autoparts_is_woocommerce_page())
...
if ( !function_exists( 'autoparts_woocommerce_merge_styles' ) ) {
	//Handler of the add_filter('autoparts_filter_merge_styles', 'autoparts_woocommerce_merge_styles');
...
if ( !function_exists( 'autoparts_woocommerce_merge_scripts' ) ) {
	//Handler of the add_filter('autoparts_filter_merge_scripts', 'autoparts_woocommerce_merge_scripts');
...
if ( !function_exists( 'autoparts_woocommerce_tgmpa_required_plugins' ) ) {
	//Handler of the add_filter('autoparts_filter_tgmpa_required_plugins',	'autoparts_woocommerce_tgmpa_required_plugins');
...
if ( !function_exists( 'autoparts_woocommerce_list_sidebars' ) ) {
	//Handler of the add_filter( 'autoparts_filter_list_sidebars', 'autoparts_woocommerce_list_sidebars' );
...
if (!function_exists('autoparts_woocommerce_pre_get_posts')) {
	//Handler of the add_action( 'pre_get_posts', 'autoparts_woocommerce_pre_get_posts' );
...
if ( !function_exists( 'autoparts_woocommerce_wrapper_start' ) ) {
	//remove_action( 'woocommerce_before_main_content', 'autoparts_woocommerce_wrapper_start', 10);
	//Handler of the add_action('woocommerce_before_main_content', 'autoparts_woocommerce_wrapper_start', 10);
...
if ( !function_exists( 'autoparts_woocommerce_wrapper_end' ) ) {
	//remove_action( 'woocommerce_after_main_content', 'autoparts_woocommerce_wrapper_end', 10);		
	//Handler of the add_action('woocommerce_after_main_content', 'autoparts_woocommerce_wrapper_end', 10);
...
if ( !function_exists( 'autoparts_woocommerce_archive_description' ) ) {
	//Handler of the add_action( 'woocommerce_archive_description', 'autoparts_woocommerce_archive_description', 15 );
...
if ( !function_exists( 'autoparts_woocommerce_before_shop_loop' ) ) {
	//Handler of the add_action( 'woocommerce_before_shop_loop', 'autoparts_woocommerce_before_shop_loop', 10 );
...
if ( !function_exists( 'autoparts_woocommerce_loop_shop_columns' ) ) {
	//Handler of the add_filter( 'loop_shop_columns', 'autoparts_woocommerce_loop_shop_columns' );
	function autoparts_woocommerce_loop_shop_columns($cols) {
		return max(2, min(4, autoparts_get_theme_option('blog_columns')));
	}
}

// Add column class into product item in shop streampage
if ( !function_exists( 'autoparts_woocommerce_loop_shop_columns_class' ) ) {
	//Handler of the add_filter( 'post_class', 'autoparts_woocommerce_loop_shop_columns_class' );
	//Handler of the add_filter( 'product_cat_class', 'autoparts_woocommerce_loop_shop_columns_class', 10, 3 );
...
if ( !function_exists( 'autoparts_woocommerce_item_wrapper_start' ) ) {
	//Handler of the add_action( 'woocommerce_before_subcategory_title', 'autoparts_woocommerce_item_wrapper_start', 9 );
	//Handler of the add_action( 'woocommerce_before_shop_loop_item_title', 'autoparts_woocommerce_item_wrapper_start', 9 );
...
if ( !function_exists( 'autoparts_woocommerce_open_item_wrapper' ) ) {
	//Handler of the add_action( 'woocommerce_before_subcategory_title', 'autoparts_woocommerce_title_wrapper_start', 20 );
	//Handler of the add_action( 'woocommerce_before_shop_loop_item_title', 'autoparts_woocommerce_title_wrapper_start', 20 );
...
if ( !function_exists( 'autoparts_woocommerce_title_tags' ) ) {
	//Handler of the add_action( 'woocommerce_before_shop_loop_item_title', 'autoparts_woocommerce_title_tags', 30 );
...
if ( !function_exists( 'autoparts_woocommerce_the_title' ) ) {
	//Handler of the add_filter( 'the_title', 'autoparts_woocommerce_the_title' );
...
if ( !function_exists( 'autoparts_woocommerce_shop_loop_subcategory_title' ) ) {
	//Handler of the add_filter( 'woocommerce_shop_loop_subcategory_title', 'autoparts_woocommerce_shop_loop_subcategory_title' );
...
if ( !function_exists( 'autoparts_woocommerce_title_wrapper_end' ) ) {
	//Handler of the add_action( 'woocommerce_after_shop_loop_item_title', 'autoparts_woocommerce_title_wrapper_end', 7);
...
if ( !function_exists( 'autoparts_woocommerce_title_wrapper_end2' ) ) {
	//Handler of the add_action( 'woocommerce_after_subcategory_title', 'autoparts_woocommerce_title_wrapper_end2', 10 );
...
if ( !function_exists( 'autoparts_woocommerce_close_item_wrapper' ) ) {
	//Handler of the add_action( 'woocommerce_after_subcategory', 'autoparts_woocommerce_item_wrapper_end', 20 );
	//Handler of the add_action( 'woocommerce_after_shop_loop_item', 'autoparts_woocommerce_item_wrapper_end', 20 );
...
if ( !function_exists( 'autoparts_woocommerce_add_to_cart_text' ) ) {
	//Handler of the add_filter( 'woocommerce_product_add_to_cart_text',		'autoparts_woocommerce_add_to_cart_text' );
	//Handler of the add_filter( 'woocommerce_product_single_add_to_cart_text','autoparts_woocommerce_add_to_cart_text' );
	function autoparts_woocommerce_add_to_cart_text($text='') {
		return esc_html__('Buy now', 'autoparts');
	}
}

// Decorate price
if ( !function_exists( 'autoparts_woocommerce_get_price_html' ) ) {
	//Handler of the add_filter(    'woocommerce_get_price_html',	'autoparts_woocommerce_get_price_html' );
...
if (!function_exists('autoparts_woocommerce_localize_script')) {
	//Handler of the add_filter( 'autoparts_filter_localize_script','autoparts_woocommerce_localize_script' );
...
if ( !function_exists( 'autoparts_woocommerce_show_product_id' ) ) {
	//Handler of the add_action( 'woocommerce_product_meta_end', 'autoparts_woocommerce_show_product_id', 10);
...
if ( !function_exists( 'autoparts_woocommerce_product_thumbnails_columns' ) ) {
	//Handler of the add_filter( 'woocommerce_product_thumbnails_columns', 'autoparts_woocommerce_product_thumbnails_columns' );
...
if ( !function_exists( 'autoparts_woocommerce_output_related_products_args' ) ) {
	//Handler of the add_filter( 'woocommerce_output_related_products_args', 'autoparts_woocommerce_output_related_products_args' );
...
if ( !function_exists( 'autoparts_woocommerce_related_products_columns' ) ) {
	//Handler of the add_filter('woocommerce_related_products_columns', 'autoparts_woocommerce_related_products_columns' );
...
if ( !function_exists( 'autoparts_woocommerce_get_product_search_form' ) ) {
	//Handler of the add_filter( 'get_product_search_form', 'autoparts_woocommerce_get_product_search_form' );
