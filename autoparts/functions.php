...
if ( !function_exists( 'autoparts_wp_scripts' ) ) {
	//Handler of the add_action('wp_enqueue_scripts', 'autoparts_wp_scripts', 1000);
...
if ( !function_exists( 'autoparts_wp_scripts_child' ) ) {
	//Handler of the add_action('wp_enqueue_scripts', 'autoparts_wp_scripts_child', 1200);
...
if ( !function_exists( 'autoparts_localize_scripts' ) ) {
	//Handler of the add_action('wp_footer', 'autoparts_localize_scripts');
...
if ( !function_exists( 'autoparts_wp_scripts_responsive' ) ) {
	//Handler of the add_action('wp_enqueue_scripts', 'autoparts_wp_scripts_responsive', 2000);
...
if (!function_exists('autoparts_wp_head')) {
	//Handler of the add_action('wp_head',	'autoparts_wp_head', 1);
...
if ( !function_exists('autoparts_add_body_classes') ) {
	//Handler of the add_filter( 'body_class', 'autoparts_add_body_classes' );
...
if ( !function_exists( 'autoparts_wp_footer' ) ) {
	//Handler of the add_action('wp_footer', 'autoparts_wp_footer');
...
if ( !function_exists('autoparts_register_sidebars') ) {
	// Handler of the add_action('widgets_init', 'autoparts_register_sidebars');
...
if ( !function_exists('autoparts_theme_fonts_links') ) {
	function autoparts_theme_fonts_links() {
		...
		*/
		$google_fonts_enabled = ( 'off' !== _x( 'on', 'Google fonts: on or off', 'autoparts' ) );
		$custom_fonts_enabled = ( 'off' !== _x( 'on', 'Custom fonts (included in the theme): on or off', 'autoparts' ) );
...
if ( !function_exists( 'autoparts_importer_set_options' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_importer_options',	'autoparts_importer_set_options', 9 );
	function autoparts_importer_set_options($options=array()) {
		if (is_array($options)) {
			// Save or not installer's messages to the log-file
			$options['debug'] = false;
			// Prepare demo data
			$options['demo_url'] = esc_url(autoparts_get_protocol() . '://autoparts.themerex.net/demo/');
			// Required plugins
			...
			$options['files']['default']['domain_dev'] = esc_url(autoparts_get_protocol().'');		// Developers domain
			...
			// 		$options['files']['dark_demo']['title'] = esc_html__('Dark Demo', 'autoparts');
		}
		return $options;
	}
}
