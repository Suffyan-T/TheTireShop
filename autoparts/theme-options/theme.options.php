	...
	function autoparts_options_create() {
		...
		for ($i=1; $i<=autoparts_get_theme_setting('max_load_fonts'); $i++) {
			$fonts["load_fonts-{$i}-info"] = array(
				"title" => esc_html(sprintf(__('Font %s', 'autoparts'), $i)),
				"desc" => '',
				"type" => "info",
				);
		...
		foreach ($theme_fonts as $tag=>$v) {
			$fonts["{$tag}_section"] = array(
				"title" => !empty($v['title']) 
								? $v['title'] 
								: esc_html(sprintf(__('%s settings', 'autoparts'), $tag)),
				"desc" => !empty($v['description']) 
								? $v['description'] 
								: wp_kses_post( sprintf(__('Font settings of the "%s" tag.', 'autoparts'), $tag) ),
				"type" => "section",
				);
