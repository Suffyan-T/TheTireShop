...
if (!function_exists('autoparts_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'autoparts_hovers_theme_setup3', 3 );
	function autoparts_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		autoparts_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'autoparts'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'autoparts') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'autoparts'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'autoparts'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'autoparts') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'autoparts'),
					'icon'	=> esc_html__('Icon',	'autoparts'),
					'icons'	=> esc_html__('Icons',	'autoparts'),
					'zoom'	=> esc_html__('Zoom',	'autoparts'),
					'fade'	=> esc_html__('Fade',	'autoparts'),
					'slide'	=> esc_html__('Slide',	'autoparts'),
					'pull'	=> esc_html__('Pull',	'autoparts'),
					'border'=> esc_html__('Border',	'autoparts')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('autoparts_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'autoparts_hovers_theme_setup9', 9 );
	function autoparts_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'autoparts_hovers_frontend_scripts', 1010 );
		add_filter( 'autoparts_filter_localize_script','autoparts_hovers_localize_script' );
		add_filter( 'autoparts_filter_merge_scripts',	'autoparts_hovers_merge_scripts' );
		add_filter( 'autoparts_filter_merge_styles',	'autoparts_hovers_merge_styles' );
		add_filter( 'autoparts_filter_get_css', 		'autoparts_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'autoparts_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'autoparts_hovers_frontend_scripts', 1010 );
	function autoparts_hovers_frontend_scripts() {
		if ( autoparts_is_on(autoparts_get_theme_option('debug_mode')) && autoparts_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'autoparts-hovers', autoparts_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( autoparts_is_on(autoparts_get_theme_option('debug_mode')) && autoparts_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'autoparts-hovers',  autoparts_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('autoparts_hovers_merge_scripts')) {
	//Handler of the add_filter( 'autoparts_filter_merge_scripts', 'autoparts_hovers_merge_scripts' );
	function autoparts_hovers_merge_scripts($list) {
...
if (!function_exists('autoparts_hovers_merge_styles')) {
	//Handler of the add_filter( 'autoparts_filter_merge_styles', 'autoparts_hovers_merge_styles' );
	function autoparts_hovers_merge_styles($list) {
...
if (!function_exists('autoparts_hovers_localize_script')) {
	//Handler of the add_filter( 'autoparts_filter_localize_script','autoparts_hovers_localize_script' );
	function autoparts_hovers_localize_script($arr) {
		$arr['button_hover'] = autoparts_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('autoparts_hovers_add_icons') ) {
	function autoparts_hovers_add_icons($hover, $args=array()) {
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'autoparts') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'autoparts');
						else
							esc_html_e('Details', 'autoparts');
					}
				?></a>
			</div>
			<?php
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php the_permalink(); ?>" aria-hidden="true" class="icon-8"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'autoparts_hovers_get_css' ) ) {
	//Handler of the add_filter( 'autoparts_filter_get_css', 'autoparts_hovers_get_css', 10, 4 );
	function autoparts_hovers_get_css($css, $colors, $fonts, $scheme='') {
