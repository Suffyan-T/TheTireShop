...
jQuery(document).ready(function() {
	...
	jQuery('.autoparts_meta_box_field_preview').on('click', '> span', function(e) {
	...
	function autoparts_show_media_manager(el) {
		...
		AUTOPARTS_STORAGE['media_frame'][AUTOPARTS_STORAGE['media_id']].on( 'insert select', function(selection) {
			...
			var preview = field.siblings('.autoparts_meta_box_field_preview');
			if (preview.length == 0) {
				jQuery('<span class="autoparts_meta_box_field_preview"></span>').insertAfter(field);
				preview = field.siblings('.autoparts_meta_box_field_preview');
			}
