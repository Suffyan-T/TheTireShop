...
if ( !function_exists( 'autoparts_cf7_tgmpa_required_plugins' ) ) {
	//Handler of the add_filter('autoparts_filter_tgmpa_required_plugins',	'autoparts_cf7_tgmpa_required_plugins');
	function autoparts_cf7_tgmpa_required_plugins($list=array()) {
		if (in_array('contact-form-7', autoparts_storage_get('required_plugins'))) {
			...
			$params = array(
					'name' 		=> esc_html__('Contact Form 7 Datepicker', 'autoparts'),
					'slug' 		=> 'contact-form-7-datepicker',
					'required' 	=> false
			);
			$path = autoparts_get_file_dir('plugins/contact-form-7/contact-form-7-datepicker.zip');
			if ($path != '')
				$params['source'] = $path;
			$list[] = $params;
		}
...
if ( !function_exists( 'autoparts_cf7_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'autoparts_cf7_frontend_scripts', 1100 );
...
if ( !function_exists( 'autoparts_cf7_merge_styles' ) ) {
	//Handler of the add_filter('autoparts_filter_merge_styles', 'autoparts_cf7_merge_styles');
