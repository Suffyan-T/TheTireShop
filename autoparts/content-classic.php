...
?><div class="<?php echo esc_attr($autoparts_blog_style[0] == 'classic' ? 'column' : 'masonry_item masonry_item'); ?>-1_<?php echo esc_attr($autoparts_columns); ?>"><article id="post-<?php the_ID(); ?>"
	...
	<div class="post_content entry-content">
		<div class="post_content_inner">
			<?php
			$autoparts_show_learn_more = false; //!in_array($autoparts_post_format, array('link', 'aside', 'status', 'quote'));
			if (has_excerpt()) {
				the_excerpt();
		...
		if ( $autoparts_show_learn_more ) {
			?><p><a class="more-link" href="<?php echo esc_url(get_permalink()); ?>"><?php esc_html_e('Read more', 'autoparts'); ?></a></p><?php
		}
		?>
	</div><!-- .entry-content -->
